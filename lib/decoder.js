...
const packetLengthFns = {
  1(buf, bufLength) {
    const type = buf[1];
    const baseLength = type === RpcCommandType.RESPONSE ? 20 : 22;
    if (bufLength < baseLength) {
      return 0;
    }
    return type === RpcCommandType.RESPONSE ?
      baseLength + buf.readInt16BE(12) + buf.readInt16BE(14) + buf.readInt32BE(16) :
      baseLength + buf.readInt16BE(14) + buf.readInt16BE(16) + buf.readInt32BE(18);
  },
  2(buf, bufLength) {
    const type = buf[2];
    const baseLength = type === RpcCommandType.RESPONSE ? 22 : 24;
    if (bufLength < baseLength) {
      return 0;
    }
    let len = type === RpcCommandType.RESPONSE ?
      baseLength + buf.readInt16BE(14) + buf.readInt16BE(16) + buf.readInt32BE(18) :
      baseLength + buf.readInt16BE(16) + buf.readInt16BE(18) + buf.readInt32BE(20);
